	...
	if (btf_ext_data) {
		...
		for (seg_num = 0; seg_num < ARRAY_SIZE(ext_segs); seg_num++) {
			...
			for_each_btf_ext_sec(seg, sec) {
				...
				sec_obj = elfio_get_section_by_name(elf, sec_name);
				if (!sec_obj)
					continue;
#endif  
#ifdef  HAVE_LIBELF
...
#elif defined HAVE_ELFIO
				seg->sec_idxs[sec_num - 1] = elfio_section_get_index(sec_obj);
#endif
			}
		}
	}
...
#elif defined HAVE_ELFIO
	obj->efile.sec_cnt = elfio_get_sections_num(elf);
#endif
...
static long elf_find_func_offset(const char *binary_path, const char *name)
{
	...
	bool is_load = elfio_load(pelfio, binary_path);
#endif  //
#ifdef  HAVE_LIBELF
	...
	for (i = 0; i < ARRAY_SIZE(sh_types); i++) {
...
#elif HAVE_ELFIO
		pSec = elf_find_next_scn_by_type(pelfio, sh_types[i], NULL);
#endif  //HAVE_LIBELF
#ifdef  HAVE_LIBELF
		if (!scn) {
#elif HAVE_ELFIO
...
#elif HAVE_ELFIO
		strtabidx = elfio_section_get_link(pSec);
#endif  //
#ifdef  HAVE_LIBELF
...
#elif HAVE_ELFIO
		realData.d_buf = (void*)elfio_section_get_data(pSec);
		realData.d_size = elfio_section_get_size(pSec);
		if (!realData.d_buf) {
			pr_warn("elf: failed to get symbols for symtab section in '%s': %s\n",
				binary_path, elf_errmsg(-1));
			ret = -LIBBPF_ERRNO__FORMAT;
		...
		nr_syms = realData.d_size / elfio_section_get_entry_size(pSec);
		psymbol_t  psymbols;
		psymbols = elfio_symbol_section_accessor_new(pelfio, pSec);
		if (!psymbols) {
			pr_warn("elf: failed to get symbols in '%s': %s\n",
				binary_path, elf_errmsg(-1));
			ret = -LIBBPF_ERRNO__FORMAT;
			goto out;
		}
		...
		for (idx = 0; idx < nr_syms; idx++) {
        	...
        	unsigned char other;
        	elfio_symbol_get_symbol( psymbols, idx, sname, 128, &st_value, &size, &bind,
...
#elif HAVE_ELFIO
			psection_t pSymSection= NULL;
			pSymSection = elfio_get_section_by_index(pelfio,section_index);
			if (!pSymSection)
				continue;
			Elf64_Addr sh_addr = elfio_section_get_address(pSymSection);
			Elf64_Off sh_offset = elfio_section_get_offset(pSymSection);
			ret = st_value - sh_addr + sh_offset;
#endif  //HAVE_LIBELF
			last_bind = curr_bind;
		}
#ifdef  HAVE_ELFIO
