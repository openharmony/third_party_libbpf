...
#elif defined HAVE_ELFIO
				obj->efile.realst_ops_link_data.d_buf = data->d_buf;
				obj->efile.realst_ops_link_data.d_size = data->d_size;
				obj->efile.st_ops_link_data = &(obj->efile.realst_ops_link_data);
#endif
					      ...
					      const char *func_name)
{
	...
	if (entry.compression) {
		...
		elf = elfio_new();
		int fdm = syscall(__NR_memfd_create, "bpfelf", MFD_CLOEXEC);
		ftruncate(fdm, entry.data_length);
		write(fdm, (char *)entry.data, entry.data_length);
		snprintf(memfd_path, PATH_MAX, "/proc/self/fd/%d", fdm);
		elfio_load(elf, memfd_path);
#endif
	if (!elf) {
